...
class Arrays {
  ...
  /**
		Performs a `foldLeft`, using the first value of `arr` as the `memo` value.
	**/
  ...
  /**
		Produces a `Pair` containing two `Array`, the left being elements where `f(e) == true`, the rest in the right.
	**/
  ...
  /**
		searches for `v` in `arr` using a predicate from `stx.Equal`
	**/
  ...
  /**
		Produces a `Pair`, on the left those elements before `index`, on the right those elements on or after.
	**/
  ...
  /**
		Produces an Array of Pair containing the value and it's index.
	**/
  ...
  /**
		As with `foldLeft` but working from the right hand side.
	**/
  ...
  /**
		Produces an `Array` of `Pair` where `Pair.t2(a[n],b[n]).`
	**/
