...
class Hasher {
  ...
  public static function getHashForType<T>(v: ValueType) : T->Int {
    return switch(v) {
      ...
      case TUnknown         : __hash__(function(v: T) {throw Error.withData('can\'t retrieve hashcode for TUnknown: $v',IllegalOperationError); return 0;});
      case TObject          :
        __hash__(function(v){
      ...
      case TFunction        : __hash__(function(v : T):Int {throw Error.withData("function can't provide a hash code",IllegalOperationError); return 0;});
      case TNull            : nil;
    }
	}
